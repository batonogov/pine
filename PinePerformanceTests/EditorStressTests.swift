...
final class EditorStressTests: XCTestCase {
    ...
    // MARK: - Large File by Size (Byte-Based Thresholds)

    /// Generates a large Swift-like file of approximately the given byte size.
    private func generateFileBySize(approximateBytes: Int) -> URL {
    ...
    func testOpen10MBFilePartialLoad() {
        let url = generateFileBySize(approximateBytes: 10 * 1_048_576)
        let tabManager = TabManager()
    ...
    func testOpen5MBFileNoHighlight() {
        let url = generateFileBySize(approximateBytes: 5 * 1_048_576)
        let tabManager = TabManager()
    ...
    func testOpen1MBFileWithHighlight() {
        let url = generateFileBySize(approximateBytes: 1_048_576)
        let tabManager = TabManager()
    ...
    func testSequentialOpen5LargeFiles() {
        let urls = (0..<5).map { _ in generateFileBySize(approximateBytes: 2 * 1_048_576) }
        let tabManager = TabManager()
