...
- **SwiftLint:** `brew install swiftlint` — runs as a build phase; config in `.swiftlint.yml`. CI pins SwiftLint 0.65.1 — the same version `brew install swiftlint` gives you locally, so a clean local run means a clean CI run (bump `SWIFTLINT_VERSION`, `SWIFTLINT_SHA256`, and `SWIFTLINT_MACOS_SHA256` in `.github/workflows/ci.yml` together; SourceKit-dependent rules run only in the macOS `SwiftLint (SourceKit rules)` lane, #1546). Run `swiftlint` before every commit and fix all warnings/errors. If `swiftlint` crashes with `sourcekitdInProc` error, prefix with `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer`
